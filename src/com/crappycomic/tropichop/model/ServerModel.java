...
import com.crappycomic.tropichop.net.Server;
import com.crappycomic.tropichop.net.ServerSideConnection;
import com.crappycomic.tropichop.view.ViewMessage;
import com.crappycomic.tropichop.view.ViewMessage.Type;
...
public class ServerModel extends Model implements Runnable
{
   ...
   {
      Collection<Node> nodes = new ArrayList<>(player.getOwnedNodes());
   ...
   private void attemptToSettleDebt()
   {
      ...
      sendMessage(Type.PLAYER_HAS_INSUFFICIENT_CASH, debtor, new Pair<>(creditor, amount));
   ...
   void roll(Player player, int rollMultiplier, boolean allowSpecialRollsAndUseFuel)
   {
      ...
      {
         allowedMovesList = new ArrayList<>(allowedMoves);
   ...
   {
      messages.add(new Pair<>(connection, modelMessage));
   }
   ...
   {
      Collection<Node> nodes = new ArrayList<>(nodeIDs.size());
