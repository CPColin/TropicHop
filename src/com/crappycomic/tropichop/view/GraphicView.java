...
import com.crappycomic.tropichop.model.ModelMessage.Type;
...
public class GraphicView extends View
{
         ...
         public void run()
         {
            ...
            if (remotePlayer && SystemTray.isSupported())
            {
               try
               {
                  ...
                  trayIcon = new TrayIcon(image, "TropicHop");
   ...
   protected void playerChangedFuel(Player player, int amount)
   {
      statusPanel.appendText(player.getName() + (amount > 0 ? " added " : " used ") + Math.abs(amount)
         + " barrel" + (amount == 1 || amount == -1 ? "" : "s") + " of rum.");
   ...
   {
      statusPanel.appendText(player.getName() + " drew a Maelstrom card: " + getCardDisplayName(card.getID()) + ".");
   }
   ...
   {
      statusPanel.appendText(player.getName() + " lost a dispute with the Company and must relinquish control of a property.");      
   }
   ...
   {
      statusPanel.appendText(player.getName() + " was forced to move with insufficient rum and has lost.");
      boardPanel.updateBoard();
      playersPanel.gameOver(player);
   }
   ...
   {
      statusPanel.appendText(player.getName() + " was marooned due to critical rum and has lost.");
      boardPanel.updateBoard();
      playersPanel.gameOver(player);
   }
   ...
   {
      statusPanel.appendText(player.getName() + " placed a still on " + getNodeDisplayName(node.getID()) + ".");
      boardPanel.updateBoard();
      nodePanel.updateNode(node);
   }
   ...
   protected void playerPurchasedFuelStation(Player player)
   {
      ...
      statusPanel.appendText(player.getName() + " purchased a still (" + remaining + " remain" + (remaining == 1 ? "s" : "") + ").");      
   }

   @Override
   protected void playerPurchasedNode(Player player, Node node)
   {
      statusPanel.appendText(player.getName() + " purchased " + getNodeDisplayName(node.getID()) + " from the Company.");
      boardPanel.updateBoard();
      playersPanel.updateTotalWorth(player);
      nodePanel.updateNode(node);
   }
   ...
   protected void playerSoldFuelStation(Player player)
   {
      ...
      statusPanel.appendText(player.getName() + " sold a still to the Company (" + remaining + " remain" + (remaining == 1 ? "s" : "") + ").");      
   }

   @Override
   protected void playerSoldNode(Player player, Node node)
   {
      statusPanel.appendText(player.getName() + " sold " + getNodeDisplayName(node.getID()) + " to the Company.");
      boardPanel.updateBoard();
      nodePanel.updateNode(node);
   }
   ...
   {
      statusPanel.appendText(player.getName() + " won a dispute with the Company and may take ownership of an unowned property.");      
   }
   ...
   protected void promptForPreRollActions()
   {
      ...
      if (trayIcon != null && !frame.isActive() && isPlayerLocal(player))
      {
         trayIcon.displayMessage("Waiting for " + player.getName(),
            "Click this balloon to bring TropicHop to the foreground.", TrayIcon.MessageType.INFO);
      }
   }
   ...
   {
      actionsPanel.showChooseNodeActions("Choose a property to relinquish to the Company:",
   ...
   {
      actionsPanel.showChooseNodeActions("Choose a property to obtain from the Company:",
   ...
   {
      statusPanel.appendText(player.getName() + " obtained a free still.");
   }
