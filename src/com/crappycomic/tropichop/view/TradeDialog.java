...
public class TradeDialog extends JDialog
{
   ...
   private JList<Node> fromList;
   
   private JList<Node> toList;
   ...
   TradeDialog(final GraphicView view, Player from, Player to)
   {
      ...
      if (!from.getOwnedNodes().isEmpty() || !to.getOwnedNodes().isEmpty())
      {
         ...
         box.add(Box.createHorizontalStrut(GAP));
         box.add(fromList = new JList<>(from.getOwnedNodes().toArray(new Node[0])));
         box.add(Box.createHorizontalGlue());
         box.add(toList = new JList<>(to.getOwnedNodes().toArray(new Node[0])));
         box.add(Box.createHorizontalStrut(GAP));
         box.add(button = new JButton("Clear"));
      ...
      button.addActionListener(new ActionListener()
      {
         ...
         public void actionPerformed(ActionEvent evt)
         {
            ...
            if (trade != null)
               view.sendMessage(ModelMessage.Type.TRADE, trade.getFrom(), trade);
         }
      });
   ...
   Trade getValue()
   {
      List<Node> offered = new ArrayList<>();
      List<Node> requested = new ArrayList<>();
      int cash = (Integer)cashSpinner.getValue();
      
      if (fromList != null)
         for (Object node : fromList.getSelectedValuesList())
            offered.add((Node)node);
      if (toList != null)
         for (Object node : toList.getSelectedValuesList())
